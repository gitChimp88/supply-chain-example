...
contract("SupplyChain", function (accounts) {
  ...
  before(async () => {
    ...
    // get access control contracts here
    // and make sure different accounts are set up with the correct controls

    farmerRole = await FarmerRole.deployed();
    await farmerRole.addFarmer(accounts[1]);

    distributorRole = await DistributorRole.deployed();
    await distributorRole.addDistributor(accounts[2]);

    const roleAssigned = await distributorRole.isDistributor(distributorID);
    console.log("has distributor role - ", roleAssigned);
  });
  ...
  it("Testing smart contract function sellItem() that allows a farmer to sell coffee", async () => {
    ...
    let itemPrice = web3_utils.toWei(".02", "ether");
    await supplyChain.sellItem(upc, itemPrice);
  ...
  it("Testing smart contract function buyItem() that allows a distributor to buy coffee", async () => {
    ...
    await supplyChain.buyItem(upc, { from: distributorID, value: itemPrice });
    // Retrieve the just now saved item from blockchain by calling function fetchItem()
    const resultBufferTwo = await supplyChain.fetchItemBufferTwo.call(upc);
    // Verify the result set
    assert.equal(eventEmitted, true, "Invalid event emitted");
